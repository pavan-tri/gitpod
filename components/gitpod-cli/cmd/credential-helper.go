...
var credentialHelper = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		log.Infof("gp credential.helper %s", action)
		log.Infof("isTheiaIDE() + %t", isTheiaIDE())
		...
		defer func() {
			...
			log.Infof("returning username=%s\npassword=%s\n", user, token)
		...
		log.Infof("Runnig gp git-token-validator --user %s --token %s --scopes %s --host %s --repoURL %s --gitCommand %s",
			resp.User, resp.Token, strings.Join(resp.Scope, ","), host, repoURL, gitCommand)
		validator := exec.Command("gp", "git-token-validator",
