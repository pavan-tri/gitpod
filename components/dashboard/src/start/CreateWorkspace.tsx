...
export interface PrebuildInProgressProps {
  runningPrebuild: {
    prebuildID: string
    workspaceID: string
...
export function PrebuildInProgress(props: PrebuildInProgressProps) {
  ...
  return <StartPage title="Prebuild in Progress">
    ...
    <button className="mt-6" onClick={() => { clearTimeout(pollTimeout!); props.onIgnorePrebuild(); }}>Don't Wait for Prebuild</button>
  </StartPage>;
}